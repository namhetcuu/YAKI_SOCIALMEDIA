...
import { uploadToCloudinary } from "../../../shared/utils/uploadToCloudiary";
      ...
      try {
        ...
        if (chatResult.payload && chatResult.payload.length > 0) {
          const otherUserIds = Array.from(
            new Set(
              chatResult.payload.flatMap((chat) =>
                chat.userIds.filter((id) => id !== userId)
              )
            )
          );
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        {(!currentChat || window.innerWidth >= 1024) && (
          <motion.div
            ...
            exit={{ opacity: 0, x: -30 }}
            className="w-full md:w-1/3 lg:w-1/4 border-r h-full flex flex-col dark:border-gray-700"
          >
            <div className="p-4 flex items-center space-x-2 mb-6">
            ...
            >
              <div className="p-4 border-b flex justify-between items-center dark:border-gray-700">
                <div className="flex items-center space-x-3">
                  <button
                    className="lg:hidden text-2xl text-gray-600 dark:text-gray-300 hover:text-black dark:hover:text-white"
              ...
              {/* Messages */}
              <div className="flex-1 overflow-y-auto p-4 bg-gray-50 dark:bg-gray-800 min-h-0">
                {loadingMessages ? (
                  <div className="flex justify-center items-center h-full">
                    <CircularProgress />
                  </div>
                ...
                <div className="flex items-center space-x-2">
                  ...
                  {selectedImage && (
                    <div className="relative">
                      ...
                      <IconButton
                        size="small"
                        className="absolute top-0 right-0 bg-gray-200 dark:bg-gray-600"
                  ...
                  <input
                    type="text"
                    className="flex-1 border rounded-full py-2 px-4 focus:outline-none dark:bg-gray-700 dark:text-white"
        ...
        {/* Empty state (chỉ hiển thị trên desktop nếu không chọn chat) */}
        {!currentChat && window.innerWidth >= 1024 && (
